...
function Tasks() {
  ...
  return (
    <div className="w-full min-h-[23rem]  border-2 p-3 rounded-3xl bg-white overflow-y-scroll ">
      {/* title start */}
      ...
      <div className="mt-3 flex flex-col gap-y-5  ">
        {tasks.map((task, index) => {
          // Listeleyecegımız her tasklar ıcın komponent olusturup her task ıcın  bır komponent dondurebılırdık.
          if (!task.isCompleted) {
            return (
              <div
                key={index}
              ...
              >
                <span className="text-[10px] w-full ml-2 underline underline-offset-2 text-[#196F3D]">
                  {task.date.taskDate}
                </span>
